...
# input file generated by filtering parsed ChIP-Atlas (ca) bed file
# from my `UpSetToolkit` project by "chr15" and "HEK293T" using
# Rscript `bed_filter`, then merging overlapping ranges (which
# were causing an error) using custom script `bed_interval_merger.R` 
ca_file_path <- "data/chip-atlas/filtered_chip-atlas_H3K27ac_HEK293T_hg38.bed"
ca_file_path <- "data/chip-atlas/subset_chr15_hg19_parsed_Histone.Kidney.100.H3K27ac.AllCell.bed.txt"
ca_file_path <- "data/chip-atlas/merged_intervals_filtered_chip-atlas_H3K27ac_HEK293T_hg19.bed"
ca_df_bed <- data.table::fread(ca_file_path, sep = "\t", header = TRUE)

### Plot chip-atlas HEK293T, H3K27ac signal track
plotSignal(
  data = ca_df_bed,
  params = params,
  x = 0.75, y = 5.5, 
...
# Add label for ChIP-Atlas track
plotText(label = "ChIP-Atlas",
         fontsize = 10,
         fontface = "bold",
         rot = 90,
         x = 0.25,
...
# Add sub-label for ChIP-Atlas track
...
ca_a_file_path <- "data/chip-atlas/merged_intervals_filtered_chip-atlas_H3K27ac_ACHN_hg19.bed"
ca_a_df_bed <- data.table::fread(ca_file_path, sep = "\t", header = TRUE)
...
annoHighlight(
  ...
  fill = "gray36",
  y = 3.125, height = 4.325, just = c("left", "top"), default.units = "inches"
)
